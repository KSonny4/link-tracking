...
1. If you forked this repo, or cloned it into a different directory from the github structure,
   ...
   ```bash
   $ find . -path ./vendor -prune -o -type f \( -name '*.go' -o -name '*.proto' \) -exec sed -i -e "s;github.com/johanbrandhorst/grpc-gateway-boilerplate;github.com/ksonny4/link-tracking;g" {} +
   ```
